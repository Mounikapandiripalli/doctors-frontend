...
function Patient() {
  ...
  const handleAddPatient = (newPatient) => {
    ...
    axios.post(' http://13.51.206.122:5000/patients', newPatient)
      .then((response) => {
  ...
  useEffect(() => {
    axios.get('http://13.51.206.122:5000/patients')
  .then((response) => {
  ...
  const handleDeletePatient = (PatientId) => {
    ...
    axios.delete(`http://13.51.206.122:5000/patients/${PatientId}`)
      .then(() => {
  ...
  const handleEditPatient = (editedPatient) => {
    ...
    axios.put(` http://13.51.206.122:5000/patients/${editedPatient.id}`, editedPatient)
      .then(() => {
